...
LG_metrics = pd.read_csv(path+"PA_LG_metrics_df.csv")
RF_metrics = pd.read_csv(path+"PA_RF_metrics_df.csv")
GB_metrics = pd.read_csv(path+"PA_GB_metrics_df.csv")
DL_metrics = pd.read_csv(path+"updated_PA_DL_metrics_df.csv")
...
for model, df in Model_Scores.items():
  ...
  # Code GS data in 0 "for absence" and 1 "for presence" into 3 columns
  GS_coded_best = make_GS(Model_best_metrics)
  print(GS_coded_best)

  # Save new dataframe in a dictionary with best metrics selected
  Best_metrics_models[model] = GS_coded_best
...
def GS_gridplot(drug,subplot_axis, label_show = True, title_pos = -0.2):
  # Create 2 new lists one for each type of features (Population structure and Gene presence)
  ...
  Drug_GS_list = [S_list, G_list]     
  
  plt.yticks(np.arange(2), ["Structure", "Accessory Genes"])
  if label_show == False:
      subplot_axis.tick_params(left = False, labelleft = False)

  orig_map = plt.cm.get_cmap('gray')
  reversed_map = orig_map.reversed()
  subplot_axis.imshow(Drug_GS_list, cmap = reversed_map)
...
gs1 = fig.add_gridspec(nrows=4, ncols=5, left=0.05, right=0.6, wspace=0.07)
# Accuracy barcharts for all models
acc_axis = fig.add_subplot(gs1[0, :])
acc_axis.set_title('Prediction of Antibiotic Resistance From P. aeruginosa Pan-Genome Data', fontsize = 20)
...
# GS gridplot charts for each drug
gs2 = fig.add_gridspec(nrows=1, ncols=5, top=0.59,bottom=-0.04,left=0.05, right=0.6, wspace=0.2)
i=0
while (i< len(drug_list)):
  for drug in drug_list:
    if i == 0:
      Drug_grid = fig.add_subplot(gs2[-1, i])
      drug_GYS = GS_gridplot(drug, Drug_grid, label_show=True, title_pos=-0.3)
      i+=1
    else:
      Drug_grid = fig.add_subplot(gs2[-1, i])
      drug_GYS = GS_gridplot(drug, Drug_grid, label_show=False, title_pos=-0.3)
      i+=1
...
plt.legend(handles=legend_elements,loc = 'lower center', bbox_to_anchor=(-2, -1), prop ={'size': 15}, borderaxespad=-2, ncol = 3)

# %%
fig.savefig('PA_pangenome_plot.jpg', bbox_inches="tight")
# %%
